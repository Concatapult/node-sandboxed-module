...
SandboxedModule.load = function(moduleId, options, trace) {
  ...
  var filename = resolveFilename(moduleId, trace[0].getFileName(), options.resolvePath);

  sandboxedModule._init(moduleId, filename, trace, options);
...
SandboxedModule.prototype._init = function(moduleId, filename, trace, options) {
  this.id = moduleId;
...
function resolveFilename (filename, originPath, customResolveFn) {
  function resolveFn (path) { return requireLike(originPath).resolve(path) };

  return customResolveFn
    ? customResolveFn(filename, resolveFn, originPath)
    : resolveFn(filename);
};
...
SandboxedModule.prototype._createRecursiveRequireProxy = function() {
  ...
  if(!this._options.sourceTransformersSingleOnly && this._options.sourceTransformers){
    options = {
      ...
      resolvePath: this._options.resolvePath,
    };
  }
  if(this._options.resolvePath) {
    options = options || {};
    options.resolvePath = this._options.resolvePath;
  }
  ...
  function createInnerSandboxedModule(requestedFilename){
      ...
      sandboxedModule._init(requestedFilename, requestedFilename, trace, options);
  ...
  function RecursiveRequireProxy(request){
    ...
    // cached modules
    var originPath = this.filename;
    var requestedFilename = resolveFilename(request, this.filename, options.resolvePath);
